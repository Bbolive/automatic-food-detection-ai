...
from utils import setup_logging, cleanup_old_files  # ← โค้ดใหม่ตัดออก แต่ของเดิมต้องเก็บไว้
...
def create_app() -> Flask:
    ...
    logger.info(
        "App created | mode=%s | db=%s",
        "debug" if ServerConfig.DEBUG else "production",
        DB_PATH,
    )
    return app
...
def register_main_routes(app: Flask) -> None:
            ...
            return jsonify(
                {
                    "success": True,
                    "filename": filename,
                    "image_url": f"/uploads/{filename}?t={int(time.time())}",  # ✅ timestamp ใน JSON
                }
            )
    ...
    def favicon():
        return (
            ...
            if (
                app.static_folder
                and os.path.exists(os.path.join(app.static_folder, "favicon.ico"))
            )
            else ("", 204)
        )
...
def register_error_handlers(app: Flask) -> None:
    ...
    def bad_request(e):
        return (
            jsonify({"success": False, "error": "Bad request", "detail": str(e)}),
            400,
        )
    ...
    def file_too_large(e):
        return (
            jsonify(
                {
                    "success": False,
                    "error": f"File too large (max {ServerConfig.MAX_UPLOAD_MB} MB)",
                }
            ),
            413,
        )
